...
# Perform the fit
import logging
fit_parameters, chisquared = fitpy.residual_fit(functional_form, x, data, parameter_constraints={'a':(-5, 5), 'b':(-5, 5), 'c':(-5, 5)}, verbosity=logging.DEBUG, max_evaluations=5000)
...
pylab.plot(x, data)
pylab.plot(x, functional_form(x, *fit_parameters))
pylab.show()
